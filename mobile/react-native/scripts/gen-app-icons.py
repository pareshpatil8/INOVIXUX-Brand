#!/usr/bin/env python3
"""
Rasterize inovixux-icon-b2c.svg (INO-82) into the Expo-managed RN icon/splash
asset set, at the source resolutions the Expo build pipeline (`expo prebuild`)
consumes to generate the actual Xcode Images.xcassets AppIcon set and Android
mipmap/adaptive-icon directories. See INO-89.
...
network access to a browser-based renderer. The source glyph
(`assets/brand/logo/inovixux-icon-b2c.svg`) is simple enough — two round-capped
strokes, one filled circle, four small filled circles, one 2-stop linear
gradient — to reproduce exactly from its own path coordinates with
PIL + numpy, at high supersampling for clean edges. If the glyph is ever
redrawn with more complex paths, redo this with a real SVG renderer instead
of extending the manual geometry below.
...
if __name__ == "__main__":
    ...
    # iOS + general app icon: opaque, edge-to-edge (OS applies its own corner
    # mask); expo prebuild derives every Xcode Images.xcassets AppIcon slot
    # from this single 1024x1024 source.
    render(1024, include_corner_circles=True, transparent=False).save(
        os.path.join(out_dir, "icon.png"))

    # Android adaptive icon foreground: transparent, glyph confined to the
    # ~66% center safe zone per Android's adaptive-icon spec (the OS masks
    # this layer to circle/squircle/etc. per launcher, and composites it over
    # the flat `backgroundColor` set in app.json — no background image needed).
    render(1024, include_corner_circles=True, transparent=True, safe_zone_scale=0.62).save(
        os.path.join(out_dir, "adaptive-icon.png"))

    # Splash: transparent glyph, displayed centered over app.json's
    # `backgroundColor` by expo-splash-screen (`resizeMode: contain`) — same
    # single-source approach the Expo splash pipeline expects, no per-device
    # full-bleed splash images to hand-generate.
    render(1024, include_corner_circles=True, transparent=True).save(
        os.path.join(out_dir, "splash-icon.png"))

    # Favicon (Expo web output, and handy for quick visual QA of this script).
    render(196, include_corner_circles=True, transparent=True).save(
