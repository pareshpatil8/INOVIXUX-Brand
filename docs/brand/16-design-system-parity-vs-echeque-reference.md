...
| **M-13** | **PrimeNG AI tooling adoption** *(NEW)* — install the Plugin; pin `llms.txt` to `specs/primeng/llms-22.1.1.txt` | ✅ **Closed** by **INO-169** — pin present at `specs/primeng/llms-22.1.1.txt` (see §2); `scripts/check-primeng-snapshot.mjs` gates the pin's integrity on every PR (`design-system.yml`) and checks upstream drift on a weekly schedule (`primeng-drift.yml`). Plugin install itself stays a per-developer/interactive step (`pnpm @primeui/cli plugin install --tool claude --library primeng`) — it needs Node 22 + network, so it cannot back CI | §2 | 
